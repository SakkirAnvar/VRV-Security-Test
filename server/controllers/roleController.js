...
export const deleteRole = async (req, res) => {
  try {
    ...
    const usersAssociated = await Users.find({ userRole: id });
    if (usersAssociated.length > 0) {
      return res.status(400).json({
        error: `Role cannot be deleted as it is associated with ${usersAssociated.length} users. Please delete the users first.`,
      });
    }
...
export const editRole = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Role successfully updated", role: existingRole });
  } catch (error) {
    res
      .status(500)
