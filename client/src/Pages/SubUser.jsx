...
const SubUser = () => {
  ...
  const { userRole, permissions, userId } = user;
  ...
  const handleDelete = async (user) => {
    if (user) {
      const confirmDelete = window.confirm(
        `Are you sure you want to delete Sub-User "${user?.userName}"? This action cannot be undone.`
      );
      if (!confirmDelete) return;
      try {
        await deleteSubUser(user?._id).unwrap();
        window.alert(`Sub-user "${user?.userName}" deleted successfully.`);
        refetch().then(() => {
