...
const EditSubUserForm = ({ isOpen, onClose, user, onEdit, editing }) => {
  ...
  const { data, isLoading: rolesLoading } = useGetAllRolesQuery();
  ...
  const validateFields = () => {
    ...
    } else if (!/^\+[0-9]{7,15}$/.test(trimmedData?.phoneNo)) {
      errors.phoneNo =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Failed to update sub-user: ", err);
    }
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 p-4 pt-10">
      <div
        className="bg-white pt-3 rounded-lg shadow-lg max-w-3xl w-full md:w-5/6 lg:w-2/3 xl:w-1/2 flex flex-col h-3/5"
        ref={modalRef}
      >
        {editing ? (
          <Loader />
